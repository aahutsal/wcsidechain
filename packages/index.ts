...
const server: FastifyInstance<Server, IncomingMessage, ServerResponse, FastifyLoggerInstance> =
  fastify({ logger: {} as FastifyLoggerOptions })
...
proxyConfig.handlers.forEach((handlerConfig: string) => {
  ...
  logger.info(`handler {preHandlerJS} docorating`, {prefix, upstream, preHandlerJS})
